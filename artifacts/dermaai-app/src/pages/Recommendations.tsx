...
import { morningProducts, eveningProducts, avoidProducts } from "@/lib/mockData";

type Budget = "All" | "Budget" | "Premium";
type Sensitivity = "All" | "Sensitive" | "Standard";

type Product = typeof morningProducts[0];
  ...
  return (
    <Card className="border-slate-100 shadow-sm bg-white dark:bg-zinc-900 hover:shadow-md transition-shadow group flex flex-col h-full overflow-hidden">
      <div className="relative aspect-square bg-slate-50 dark:bg-zinc-800 overflow-hidden p-6 flex items-center justify-center">
        ...
        <div className="w-full h-full flex items-center justify-center text-muted-foreground bg-slate-100 dark:bg-zinc-700 rounded-xl group-hover:scale-105 transition-transform duration-500">
           {/* Fallback box since we don't have the real image */}
...
export default function Recommendations() {
  ...
  const filteredMorning = filter(morningProducts);
  const filteredNight = filter(eveningProducts);
