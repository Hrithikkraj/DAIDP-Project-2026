...
interface AppLayoutProps {
  ...
  activeTab?: "dashboard" | "scan" | "profile" | "recommendation";
}
        ...
        {mobileMenuOpen && (
          <div className={cn("md:hidden fixed inset-0 top-16 z-40 border-b animate-in fade-in slide-in-from-top-4", dark ? "bg-zinc-950 border-zinc-800" : "bg-background border-border")}>
            <nav className="flex flex-col p-4 gap-2">
              {navLinks.map((link) => (
                <Link key={link.id} href={link.href} onClick={() => setMobileMenuOpen(false)}
              ...
              <div className="flex items-center gap-2 p-4">
                <button onClick={() => setDark(d => !d)} className="flex items-center gap-3 text-base font-medium text-muted-foreground w-full">
                  {dark ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
